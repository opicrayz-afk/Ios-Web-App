...
Số lượng Web (Tabs): Nhập 1 nếu chỉ làm app 1 trang. Nhập 2 trở lên để bật tính năng Đa Tab (Multi-Tab). Sau đó nhập Tên hiển thị và Link Website cho từng trang.

(Tính năng Độc quyền: Dựa vào số tab bạn chọn, App sẽ tự động dùng giao diện Nút Nổi Kính Mờ (1 Tab) hoặc Thanh Tab Bar Liquid Glass tự động ẩn (Nhiều Tab). Đồng thời tự động tích hợp trang Cài đặt nội bộ cho phép bạn Đổi Icon trực tiếp, Sửa link, Thêm Tab mới và cài extension Tampermonkey riêng cho từng web!)
...
Tính năng Thoát ứng dụng (Exit App):
...
⚠️ LƯU Ý QUAN TRỌNG: Khi bạn chọn các quyền này, tool sẽ hiện cảnh báo đỏ. Đây là các quyền can thiệp sâu, BẮT BUỘC phải có tệp chứng chỉ .entitlements và tài khoản Apple Developer 99$/năm. Nếu bạn không có chứng chỉ mà vẫn ấn y để cố tình build, app sẽ bị văng (Crash) ngay khi mở! Hãy chọn n nếu bạn dùng chứng chỉ miễn phí (Sideloadly/TrollStore) nếu bạn có chứng chỉ mua thì có thể ấn y.
...
Number of Web Tabs: Enter 1 for a single-site app. Enter 2 or more to enable Multi-Tab mode. Then enter the Display Name and URL for each tab.

(Exclusive Feature: Based on your tab count, the app will auto-switch between a Frosted Glass Floating Button (1 Tab) or an Auto-Hiding Liquid Glass Tab Bar (Multi-Tab). It also integrates an In-App Settings page allowing you to Change the Icon instantly, Edit links, Add new tabs, and inject Tampermonkey scripts per tab!)
...
Customize the offline HTML page:
